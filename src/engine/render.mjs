...
export function renderResults(mode, results, { focus, target = 0 } = {}) {
  ...
  return rows
    .map(([r, i]) => {
      ...
      // &ensp; spaces the Best tag and lets it wrap under the name without an indent.
      const head = `<span class="result-main"><span class="rank" aria-hidden="true">${i + 1}</span><span class="pname">${esc(r.short)}${tag ? `&ensp;${tag}` : ''}</span>${figureFor(mode, r)}</span>
